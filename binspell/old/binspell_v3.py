...
def draw_text(cntrx, cntry, h, w, symbolList):
	...
	if len(symbolList) == 2:
		gv._canvas.create_text(cntrx, cntry, text=symbolList[1].upper(), font=font, tag='text')
			...
			for item2 in symbolList:
				#print item2[1]
				#print "len(str(item2[1])):", len(item2[1])		
				#if item2 not in bgrph._suggSymbList:  #no redundant chars
					#print item2
					#print item2[1] not in bgrph._suggSymbList
					#print #########
				if len(str(item2[1])) > 1:  #SPC or DEL, etc
					color = 'cyan'
					fontSz = 20
					font = 'Courier %i bold' %fontSz
					xtraSymbSpace = len(str(item2[1])) * fontSz/2
					pos = 'w'  #position of text anchor - west
				else:
					fontSz = 30
					font = 'Courier %i bold' %fontSz
					color = 'black'	
				...
				gv._canvas.create_text(x+padx, y+pady, text=item2[1].upper(), font=font, tag='text', fill=color, anchor=pos)
...
def saveState(hi,norm):
	global gv, bgrph, stack
	stateObj = State(gv._box1Freqs, gv._box2Freqs, gv._suggested, gv._huffSuggest, bgrph._suggSymbList, hi, norm)
...
def getPrevState():
	global gv, bgrph, stack
	stateObj = stack._pop()
	gv._box1Freqs = stateObj._box1
	gv._box2Freqs = stateObj._box2
	gv._suggested = stateObj._suggested
	gv._huffSuggest = stateObj._huff
	bgrph._suggSymbList = stateObj._bgraph
...
def update(decision):
	...
	if decision == left:
		...
		if len(gv._box1Freqs) == 2:
			...
			if '[DEL]' in gv._box1Freqs:
				return2PrevState()
				return
			else:
				saveState(hilite,norm)
				gv._box1Freqs = []
				gv._box2Freqs = []
