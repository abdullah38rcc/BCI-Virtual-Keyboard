...
class GlobalVariables:
	def __init__(self):
		...
		self._canColor = 'yellow'							#canvas color
		...
		self._testList = []									#for testing


	def _swap_dictionary(self, original_dict):
		"""
		This function switches the key:value pairs in a dictionary to value:key pairs
		Args: dict{key:value}
		Rtns: dict{value:key}
		"""
	...
	def _sortByValue(self, unsrtdDict):
		"""
		This function sorts dictionay items by key from greatest to least
		Args: unsorted dictionary
		Rtns: sorted list[(key,val),(key,val)...]
		"""
