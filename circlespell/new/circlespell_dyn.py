# dynamic version of circlespell
...
def update(decision):
			...
			print "in update()"
			#print "gv._circleList = ",gv._circleList[gv._highlighted]
			print "decision is: %i at time: %f\n" %(decision,time.time())
...
def update_circs(decision,x,y,r1,max_r2):
				...
				if decision == select:
					print "from update_circs: %s selected at time %f\n" %(gv._circleList[gv._highlighted],time.time())
					...
					print "from update_circs: %s now highlighted at time %f\n" %(gv._circleList[gv._highlighted],time.time())
...
def keyCtrl(event):
	...
	if event.keysym == 'Up':
		##print "key up"
		decision = 1	#select
		print "up-arrow key selected at: %f" %time.time()
	...
	while True:
		'''for key in pygame.event.get():
			if key.type == pygame.KEYUP:
				...
				update(0)'''
		update(0)
		print "back in start(): waiting 20 seconds\n"
		time.sleep(20)
