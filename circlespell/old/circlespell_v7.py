...
def update_circs(decision,x,y,r1,max_r2):
				...
				if decision == select:
					...
					set_layout()	#arrange items in circles and determine next default highlighing
					#gv._canvas.update()
...
#####################-----------------functions dealing with feedback to user-------############
...
def setCircs():
	...
	circle = ['DEL']	#start with circle2
	for item in tmpLst:
		#print item
		if len(circle) >= numSymbs:
			numSymbs += 1
			if numSymbs > maxNumSymbs:
				numSymbs = maxNumSymbs
			gv._circleList.append(circle)
			circle = []
		circle.append(item[0])
	gv._circleList.append(circle)
	gv._circleList.insert(0,['SPC'])



################------------------functions dealing with stack---------########
def push(item):
	#print "in push()"
	#global stack
	#global circle_list
